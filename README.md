...
## Running elul in Quarkus dev mode
...
```
(elul should be included in the returned array)
...
[elul-schema.json](./src/main/resources/schemas/elul-schema.json ), run the following to create an
instance of the elul workflow:
```shell script
curl -v -X POST -H "Content-Type: application/json" http://localhost:8080/elul -d@input.json
```
